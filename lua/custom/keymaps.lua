...
local magic_insertpair = function(char)
    ...
    if close_set[char] and after == char then return '<Right>' end
    if quote_set[char] and should_pair then return char .. char .. '<Left>' end
    return char
end

local function magic_delpair()
    local line = vim.api.nvim_get_current_line()
    local col = vim.fn.col('.')
    -- 光标在中间时：col - 1 是左边字符，col 是右边（当前光标下）字符
    local left = line:sub(col - 1, col - 1)
    local right = line:sub(col, col)
    
    local pairs = { ['('] = ')', ['['] = ']', ['{'] = '}', ['"'] = '"', ["'"] = "'", ['`'] = '`' }
    
    if pairs[left] and pairs[left] == right then
        return '<Del><BS>'
    end
    return '<BS>'
end
